...
const departmentOptions = ["Departments", "Sales", "finance", "Marketing", "Technical",];
...
export default function AllUser() {
    ...
    const handleDelete = async (user) => {
        if (user.is_admin || !user.is_active) { alert("you cannot delete ") } else {
    ...
    const onDepartmentChange = (user, value) => {
        try {
            ...
            axios.request(config)
                .then((response) => {
                    console.log(JSON.stringify(response.data));
                    const temp = data.map((u) => { return u.id === user.id ? updatedUser : u })
                    setData(temp)
    ...
    return (
        <div style={{ background: '#F4F4F5' }} className='min-h-full'>
            <div className='flex mr-32 justify-around '>
                {/* ... Widgets ... */}
                <Widget className='bg-white shadow-lg ml-10 mt-4 ' label="Total Accounts" count={data.length} />
                <Widget className='bg-white shadow-lg ml-10 mt-4' label="Active Account" count={activeAccounts.length} />
                <Widget className='bg-white shadow-lg ml-10 mt-4' label="Inactive Account" count={inActiveAccounts.length} />
            </div>
            <div className="activeagent rounded-lg bg-white shadow-lg ml-10 mr-44 mt-8 m-auto p-4">
                ...
                <table className='table-auto text-left w-full mt-8 border-collapse h-52'>
                    <thead className='bg-[#3E97CF] py-4 border'>
                        <tr className='h-14 p-4 border'>
                            <th className='border border-#B9B9B9 pl-4'>Name</th>
                            <th className='border pl-4'>Email</th>
                            <th className='border pl-4'>Phoneno</th>

                            <th className='border pl-4'>Department</th>
                            <th className='border pl-4'>Details</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((val, index) => (
                                    ...
                                    <tr key={`${index}-edit`} className='border '>
                                        ...
                                        <td colSpan="2" className='border pl-5 py-1'>
                                            <input
                                                ...
                                                onChange={(e) => handlePhonenoChange(e.target.value, index)}
                                            />
                                        </td>
                                        <td className='pl-8 justify-center py-2 flex'>
                                            <button
